...
# <a name="troubleshooting-the-shopify-and-business-central-synchronization"></a>Felsöka synkroniseringen av Shopify och Business Central
...
- **Alla** - Logga begäran/svarsparen för alla transaktioner, inklusive de som lyckades. Kontinuerlig loggning av alla fel kan komma att göra [!INCLUDE [prod_short](../includes/prod_short.md)] långsammare. Använd det här läget när datautbytet inte resulterar i fel, men du vill få mer insikt om den data som faktiskt skickades och togs emot. Observera att vissa data alltid loggas, oavsett om loggning är påslagen. Mer information finns i [Datainsamling](#data-capture).
...
### <a name="manage-log-entry-data"></a>Hantera loggpostdata
...
Du har använt [bädda in appen](/dynamics365/business-central/dev-itpro/deployment/embed-app-overview), där klientens URL har formatet: `https://[application name].bc.dynamics.com`. Shopify anslutningen fungerar inte för inbäddade appar. För mer information, se [Vilka Microsoft-produkter är Shopify-anslutningen tillgängliga för?](shopify-faq.md#which-microsoft-products-is-the-shopify-connector-available-for).
...
Användare som begär åtkomst inte har rättigheter att hantera appar (möjlighet att hantera och installera appar och kanaler, samt eventuellt godkänna appavgifter). Du kanske kan lösa detta genom att installera appen som kontoägare. Du kan också kontrollera **Appbehörigheten** för användaren i inställningarna [**Användare och behörigheter**](https://www.shopify.com/admin/settings/account) i din **Shopify administratören**.  

### <a name="messageaccess-denied-for-field-fieldlocationsline0column0pathpathextensionscodeaccess_denieddocumentationhttpsshopifydevapiusageaccess-scopes"></a>[{"meddelande":"Åtkomst nekad för fältet FÄLT.","platser":[{"rad":0,"kolumn":0}],"sökväg":["sökväg"],"tillägg":{"kod":"ÅTKOMST_NEKAD","dokumentation":https://shopify.dev/api/usage/access-scopes}}]
...
### [API] Ogiltig API-nyckel eller åtkomsttoken (okänd inloggning eller fel lösenord)
