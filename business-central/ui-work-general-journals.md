...
De flesta journaler baseras på redovisningsjournalen och du kan behandla alla transaktioner på sidan **Redovisningsjournaler** . Läs mer på [Bokföra transaktioner direkt i redovisningen](finance-how-post-transactions-directly.md).  
...
Men erbjuder även journaler som är optimerade för specifika typer av transaktioner, [!INCLUDE [prod_short](includes/prod_short.md)]  till exempel betalningsjournalen *för* registrering av betalningar. Mer information finns i [Registrera betalningar och återbetalningar i betalningsjournalen](payables-how-post-payments-refunds.md).  
...
## <a name="add-context-to-general-journal-transactions"></a>Lägga till kontext i redovisningsjournaltransaktioner
...
Det finns flera redovisningsjournalmallar. Varje journalmall representeras av en särskild sida med särskilda funktioner och de fält som krävs för att stödja dessa funktioner, till exempel **sidan Betalningsavstämningsjournaler** för att behandla bankbetalningar och sidan **Utbetalningsjournaler** för att betala dina leverantörer eller ersätta dina anställda. Mer information finns i [Gör betalningar](payables-make-payments.md) och [Stäm av kundbetalningar med inbetalningsjournalen eller från kundreskontratransaktioner](receivables-how-apply-sales-transactions-manually.md).
...
Du kan aktivera en bakgrundskontroll som förhindrar fördröjningar vid bokföring. Kontrollen meddelar dig när ett misstag i den finansiella journalen som du arbetar med hindrar dig från att bokföra journalen.  **På sidan Redovisningsjournaler** kan du välja **Bakgrundsfelskontroll** om du vill att [!INCLUDE[prod_short](includes/prod_short.md)] redovisningsjournaler, till exempel redovisnings- eller utbetalningsjournaler, ska valideras medan du arbetar med dem.

När du aktiverar valideringen visas faktaboxen **Kontrollera dokument** eller **Journalkontroll** på den aktuella raden och i hela partiet. Valideringen sker när du läser in en finansiell journal och väljer en annan journalrad. Panelen **Ärenden totalt** i faktaboxen visar det totala antalet ärenden som [!INCLUDE[prod_short](includes/prod_short.md)] hittade, och du kan välja att den öppnar en översikt över ärendena.
...
| Till | Gå till |
| --- | --- |
|F Fast|Journalradens belopp står kvar när du har bokfört. Rader med beloppen noll finns kvar i journalen, men bokförs inte. Du kan uppdatera beloppet vid en senare körning.  |
|V Variabel|Journalradens belopp tas bort när du har bokfört.|
...
|AB Återföring balansering|Det bokförda beloppet på kontot på raden kommer att fördelas på de konton som angetts för raden på sidan **Fördelningar**. Saldot på kontot anges till noll, och en mottransaktion bokförs på nästa dag.|
|Saldo per dimension|Journalraden fördelar kostnader baserat på ett redovisningskontos saldo efter dimension. Du uppmanas att ange de dimensionsfilter som ska användas för att beräkna källredovisningskontots saldo efter dimension som du vill fördela kostnader från. Som ett alternativ kan du välja åtgärden **Dimensionsfilter** senare.|
...
Om du vill bokföra en transaktion till den sista dagen i varje månad kan du göra på någon av följande åtgärder:
...
Fältet bestämmer vilket datum raden ska bokföras för sista gången. Raden kommer inte att bokföras efter detta datum.
 ...
 **På sidan Återkommande redovisningsjournaler** kan du välja **åtgärden Fördelningar** om du vill ange hur beloppen på raden i den återkommande journalen ska fördelas på flera konton och dimensioner. Fördelningen fungerar som en balanskontorad visavi den återkommande journalraden.
...
> [!NOTE]
> Du kan inte använda fördelningskonton i journaler som har ett arbetsflöde för godkännande för att skapa journalrader.
...
### <a name="calculate-the-reversal-date"></a>Beräkning av återföringsdatum
...
Omnumreringen av verifikationsnummer skall respektera relaterade tillämpningar, som en ansökan om betalningstillämpning som har gjorts från dokumentet på journalraden till ett leverantörskonto. I enlighet med detta kan fälten **Koppla till ID** och **Koppla till ver.nr.** i de berörda transaktionerna uppdateras.

### <a name="to-renumber-documents-in-journals"></a>Numrera om dokument i journaler

Följande procedur baseras på sidan **Redovisningsjournaler**, men gäller för alla andra journaler som baseras på redovisningsjournalen, till exempel **sidan Utbetalningsjournaler** .

1. Välj ![glödlampan som öppnar funktionen Berätta.](media/ui-search/search_small.png "Berätta vad du vill göra") anger du **redovisningsjournaler** och väljer sedan relaterad länk.
