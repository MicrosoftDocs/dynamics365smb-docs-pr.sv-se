...
För att kontrollera om det antal som är registrerat i programmet är samma som det antal som finns i lagret utför du fysisk inventering. Det vill säga, inventera de faktiska artiklarna till hands. Normalt görs inventeringar i slutet av ett räkenskapsår, men ibland inventerar företag artiklar oftare. Om det finns avvikelser bokför du de faktiska kvantiteter på artikelkontona innan du gör lagervärderingen.
...
- Skriv ut rapporten du vill använda för räkningen.
...
2. Välj åtgärden **Beräkna lager** från **Förbered**.
...
1.  **På sidan Inventeringsjournaler** som innehåller det beräknade förväntade lagret väljer du **åtgärden Skriv ut** från **Startsidan**.
2. På sidan Inventeringslista **anger** du om det beräknade antalet och lagerartiklarna ska visas efter serie- och partinummer i rapporten.
...
1. På varje rad på **sidan Inventeringsjournaler** där det faktiska lagersaldot, enligt inventeringen, skiljer sig från det beräknade antalet, anger du det faktiska lagersaldot **i fältet Antal inventerat** .
...
2. Om du vill justera de beräknade kvantiteterna till de faktiskt räknade kvantiteterna väljer du **åtgärden Bokför** från **startsidan**.

    Bokföring skapar artikeltransaktioner och inventeringstransaktioner. Öppna sidan Artikelkort för att hitta de resulterande inventeringstransaktionerna.
...
4. Du verifierar antalet genom att öppna **sidan Artikel kort**  för artikeln och välja **åtgärden Inventeringstransaktioner** från **Artikel**
1. .
...
3. Ställ in filtren för att ange de artiklar som ska räknas i journalen och välj **OK**.
   ...
   Om du endast har tid att räkna artikeln på vissa lagerplatser kan det uppstå avvikelser. Bokför dem senare på sidan**Aartikeljournal** med hjälp av åtgärden **Beräkna distributionslagerjustering**.  
...
När du registrerar fysiskt lager bokför du inte på artikel-, inventerings- eller värdetransaktionerna. Posterna är dock tillgängliga för avstämning när de behövs. Om du vill behålla antalet exakta kvantiteter efter att artiklar på alla lagerplatser har beräknats bokför du resultatet.<!--physical inventory journal--> Mer information finns i [Synkronisera antal i artikeltransaktioner och distributionslager](inventory-how-count-adjust-reclassify.md#to-synchronize-the-adjusted-warehouse-entries-with-the-related-item-ledger-entries).

## <a name="to-do-cycle-counting"></a>Utför cyklisk inventering

Du kan räkna artiklar så ofta du vill. Till exempel för att de är mer värdefulla eller toppsäljare. Ange inventeringsfrekvensen genom att tilldela artiklarna särskilda cyklisk inventeringsperioder.
...
På sidan **Artikeljournaler** kan du bokföra artikeltransaktioner direkt för att justera lagret för inköp, försäljning och positiva eller negativa förändringar utan att använda dokument.
...
4. På snabbfliken **Alternativ** fyller du fältet **Verifikationsnr** med ett nummer. Numret visas på artikeljournalraderna.
5. Välj **OK**. Positiva och negativa justeringarna summeras för varje artikel, och rader skapas i artikeljournalen.  
6. Bokför journalraderna för att ange kvantitetsavvikelserna i artikeltransaktionerna. Kvantiteterna på lagerplatserna och artikeltransaktionerna är nu matchade.  
